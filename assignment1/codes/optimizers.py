...
class SGD(Optimizer):
    ...
    def __init__(self, lr=0.01, momentum=0, decay=0, sheduler_func = None, beta=0.1):
        """Initialization
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            self.moments[k] = (1 - self.beta) * prev_moments[k] + self.beta * xs_grads[k]
...
class Adam(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            ...
            moments_k_hat = self.moments[k] / (1. - self.beta_1 ** (iteration+1))
            accumulators_k_hat = self.accumulators[k] / (1. - self.beta_2 ** (iteration+1))

            new_xs[k] = xs[k] - self.lr * moments_k_hat / (np.sqrt(accumulators_k_hat) + self.epsilon)
        return new_xs
